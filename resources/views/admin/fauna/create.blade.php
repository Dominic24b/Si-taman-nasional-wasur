...
@php($pageTitle = 'Tambah Fauna')

@section('content')
<form action="{{ route('admin.fauna.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl border border-green-100 p-6 max-w-2xl space-y-5">
    @csrf
        ...
        <div>
            ...
            <select name="status_konservasi" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                ...
                @foreach ($statusOptions as $key => $label)
                    <option value="{{ $key }}" @selected(old('status_konservasi') === $key)>{{ $label }}</option>
                @endforeach
            </select>
        </div>
    </div>
